...
PieChart.defaultSettings = {
  ...
  filterRange:
    "[[0, 33.99], [34, 34.9999], [35, 35.9999], [36, 36.9999], [37, 37.9999], [37.5, 50]]",
  labelList: '["-34","34-","35-","36-","37-","37.5-"]',
  colorList:
    '["#ffc93c","#dbf6e9","#9ddfd3","#31326f","#d9d9d8","#536162","#424642","#c06014","#e40017","#70C1B3"]',
};
...
function PieChart(settings, options) {
  ...
  this.colorPallete = COLOR_DEFAULT_LIST;
  if (this.settings.colorList != undefined && this.settings.colorList !== null &&
      this.settings.colorList != 'null' && this.settings.colorList) {
    this.colorPallete = JSON.parse(this.settings.colorList);
  }
  ...
  this.svg = d3.select(this.el).append("svg").attr("class", "svgWrapper");
  ...
  this.svg.append("g").attr("class", "piechart__labels");
...
PieChart.prototype.resize = function (options) {
  ...
  var xRatio =
    (options.width - this.margin.left - this.margin.right) / this.pieChartWidth;
  var yRatio =
    (options.height - this.margin.top - this.margin.bottom) /
    this.pieChartHeight;
...
PieChart.prototype.change = function (data) {
  ...
  /* ------- PIE SLICES -------*/
  var slice = this.base
    .select(".piechart__slices")
    .selectAll("path.piechart__slice")
  ...
  legends
    ...
    .attr("x", function (d, i) {
      if (lengthData <= 4) {
        if (i != 0) {
          defaultXText += legendSize * 2 + 70;
        }
        return defaultXText;
      } else {
        if (i != 0) {
          defaultXText += legendSize * 2 + 70;
          if (i == Math.ceil(lengthData / 2)) {
            defaultXText = 0;
          }
        }
        return defaultXText;
  ...
  legends
    ...
    .attr({
      ...
      x: function (d, i) {
        if (lengthData <= 4) {
          if (i != 0) {
            defaultXRect += legendSize * 2 + 70;
          }
        } else {
          if (i != 0) {
            defaultXRect += legendSize * 2 + 70;
            if (i == Math.ceil(lengthData / 2)) {
              defaultXRect = -(legendSize * 2);
            }
          }
        }
        return defaultXRect;
      ...
      y: function (d, i) {
        let y = that.margin.top - legendSize;
        if (lengthData > 4) {
          if (i >= Math.ceil(lengthData / 2)) {
            y += 24;
          }
        ...
        return y;
        // return that.margin.top + i * 28 - legendSize;
      },
      width: legendSize,
      height: legendSize,
      fill: function (d, i) {
